...
class userIssuedBooks extends Component {
    constructor(props) {
        super(props)
    ...
    changeIndexHandler(ChangeIndexTo) {
        ...
        this.props.dispatch(bookActions.getUserIssuedBooks("", newIndex, this.props.user.user_id));
        this.setState({ userBookList: this.props.books.userIssuedBooks })
    }


    navigator() {
        var pageIndexList = []
        if (this.props.books.userIssuedBookIndex < 1) {
            pageIndexList = [0, 1, 2]
        }
        else if (this.props.books.userIssuedBookIndex === 1)
            pageIndexList = [0, 1, 2]

        else if (this.props.books.userIssuedBookIndex === 2)
            pageIndexList = [1, 2, 3]
        else
            pageIndexList = [this.props.books.userIssuedBookIndex - 1, this.props.books.userIssuedBookIndex, this.props.books.userIssuedBookIndex + 1]

        return <nav aria-label="..." >

            <ul className="pagination justify-content-center pagination-lg" >
                <li className="page-item ">
                    <a href="/#"  className={this.props.books.userIssuedBookIndex ? "page-link page-mover" : "page-link page-mover disabled"} onClick={() => this.changeIndexHandler(this.props.books.userIssuedBookIndex - 1)}> <i className="fas fa-arrow-circle-left fa-2x"></i></a>
                </li>
                <li className={this.props.books.userIssuedBookIndex === pageIndexList[0] ? "page-item active" : "page-item "}><a href="/#" className="page-link " style={{ height: '45px', lineHeight: 'inherit' }} onClick={() => this.changeIndexHandler(pageIndexList[0])} >{pageIndexList[0] + 1}{this.props.books.userIssuedBookIndex === pageIndexList[0] ? <span className="sr-only">(current)</span> : ""}</a></li>
                <li className={this.props.books.userIssuedBookIndex === pageIndexList[1] ? "page-item active" : "page-item "}><a href="/#" className="page-link" onClick={() => this.changeIndexHandler(pageIndexList[1])} style={{ height: '45px', lineHeight: 'inherit' }}>{pageIndexList[1] + 1}{this.props.books.userIssuedBookIndex === pageIndexList[1] ? <span className="sr-only">(current)</span> : ""} </a></li>
                <li className={this.props.books.userIssuedBookIndex === pageIndexList[2] ? "page-item active" : "page-item "}><a href="/#" className="page-link" style={{ height: '45px', lineHeight: 'inherit' }} onClick={() => this.changeIndexHandler(pageIndexList[2])}>{pageIndexList[2] + 1}{this.props.books.userIssuedBookIndex === pageIndexList[2] ? <span className="sr-only">(current)</span> : ""}</a></li>
                <li className="page-item">
                    <a className="page-link page-mover" onClick={() => this.changeIndexHandler(this.props.books.userIssuedBookIndex + 1)} href="/#"><i className="fas fa-arrow-circle-right fa-2x"></i></a>
                </li>
            </ul>
        </nav>
    }
    ...
    componentDidMount() {
        this.props.dispatch(bookActions.getUserIssuedBooks("", 0, this.props.user));
        this.setState({ userBookList: this.props.userBooks })

    }

    componentDidUpdate() {
        if (this.props.books.userIssuedBookFound && this.props.books.userIssuedBooks !== this.state.userBookList)
            this.setState({ userBookList: this.props.books.userIssuedBooks })
    ...
    BookCardGenerator(Details) {
        ...
        var book_card =
            <div className="book-data shadow-box" key={Details['Book_id']} style={Details['available_now'] ? { opacity: 1, borderColor: 'white', display: 'flex', flexDirection: 'column', maxWidth: '300px', borderWidth: '1px', borderStyle: 'solid' } : { opacity: 1, borderColor: 'black', display: 'flex', flexDirection: 'column', maxWidth: '300px', borderWidth: '1px', borderStyle: 'solid' }} >
                <img style={{ margin: '0 auto 10px auto', maxWidth: '180px' }} alt="file not uploaded" src={this.props.route +"/images/books/" + Details['img_id']} />
...
const connectedUserBooks = connect(mapStateToProps)(userIssuedBooks);
export { connectedUserBooks as UserIssuedBooks };
